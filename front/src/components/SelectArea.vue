<template>
    <div id="SelectArea">
        ...
        <b-input @input="$emit(type + 'Param', $event)" :placeholder="getParamName()"></b-input>
        <b-button @click="$emit('previous'), $emit('save')">Précédent</b-button>
        <b-button @click="$emit('next'), $emit('save'), $router.push(type == 'action' ? 'reaction' : 'overview')">Suivant</b-button>
    </div>
</template>
...
export default vue.extend({
    ...
    methods: {
        getParamName() {
            if (this.area[this.type + 'Id'] == -1 || this.services[0] == null)
                return;
