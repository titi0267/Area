<template>
    <div id="NavBar" :class="{ 'darkMode' : $store.getters.darkMode }">
        ...
        <div class="links">
            <a @click="$router.push('/home')" :class="{ 'link' : $route.path == '/home' }">Home</a>
            <a @click="createRedirect()" :class="{ 'link' : $route.path.includes('create') }">Create</a>
        </div>
        <div class="right">
            <div class="APK">
            ...
            <div class="profile">
                <b-dropdown
                    ...
                    <b-dropdown-item custom aria-role="profileItem" class="dropUser" :class="{ 'darkMode' : $store.getters.darkMode }">
                        Logged as <b> {{ user.firstName }} {{ user.lastName }} </b>
                    </b-dropdown-item>
                    <hr class="dropdown-divider">
                    <b-dropdown-item value="profile" class="dropItem" :class="{ 'darkMode' : $store.getters.darkMode }" @click="$router.push('/profile')">
                        <b-icon icon="address-card"></b-icon>
                        Profile
                    </b-dropdown-item>
                    <b-dropdown-item value="services" class="dropItem" :class="{ 'darkMode' : $store.getters.darkMode }" @click="$router.push('/services')">
                        <b-icon icon="link"></b-icon>
                        Services
                    </b-dropdown-item>
                    <hr class="dropdown-divider">
                    <b-dropdown-item class="dropItem" :class="{ 'darkMode' : $store.getters.darkMode }" @click="setDarkMode()">
                        <b-icon :icon="$store.getters.darkMode ? 'sun' : 'moon'"></b-icon>
                        Dark Mode
                    </b-dropdown-item>
                    <hr class="dropdown-divider">
                    <b-dropdown-item value="logout" :class="{ 'darkMode' : $store.getters.darkMode }" aria-role="profileItem" @click="logout()" class="dropItem">
...
export default vue.extend({
    ...
    mounted() {
        ...
        let dropdown: HTMLCollection = document.getElementsByClassName('dropdown-content');
        if (this.$store.state.darkMode && dropdown)
            dropdown.item(0).classList.add('darkMode')
        else if (dropdown)
            dropdown.item(0).classList.remove('darkMode')
    },
    watch: {
        /**
